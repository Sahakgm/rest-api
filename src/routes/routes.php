 ...
 *  +
 * mysite.local/user
 * bolor userneri anun azganunn e berum
 */
 ...
 * user-i masin informacia e berum
 */
 ...
 *  +
 * mysite.local/delete/
 * stugum e ete user_status=1, jnjum e
 * Param 1 - name
 */
...
/*$app->get('/img/{name}', function($request, $response){
    ...
    if(!$result){
        ...
        $sql = "SELECT * FROM photos VALUE WHERE id = :id";
        $stmt = $conn->prepare($sql);
        $stmt->execute(['id' => $id]);
        $res = $stmt->fetch(PDO::FETCH_ASSOC);
        echo "<img src='../uploads/" . $res['img'] . "'>";
    }
});*/
